...
class SECTION_DUNG():
    def __init__(self, doc, build_ele, column_points, ref_point, offset, current_pos, inc_top=True, inc_bot=True):
        self.doc           = doc
    ...
    def On_Parameter(self):
        ...
        self.concr_cover    = float(self.build_ele.Concr_Cover.value/10)
        ...
        self.scale          = int(self.build_ele.Scale.value)
        self.factor         = self.scale/25
    ...
    def Reinf_label(self):
        ...
        #main stirrup
        labels.append(self.Reinf_label_dimension(self.current_pos+11, self.width + 150*self.factor, self.height/2, self.text_props, vec_int))
        #top stirrup
        if self.inc_top:
            labels.append(self.Reinf_label_dimension(self.current_pos+12, self.width + 150*self.factor, self.height - 250, self.text_props, vec_int))
        #bottom stirrup
        if self.inc_bot:
            labels.append(self.Reinf_label_dimension(self.current_pos+13, self.width + 150*self.factor, 250, self.text_props, vec_int))
    ...
    def Sec_label(self):
        ...
        text = "%U{" + "Stütze {}".format(self.column_name) + "%U}" + "\nb/h={}/{}cm, {}, B000A\nBetondeckung: Cv={} cm\nM 1:{}"\
                        .format(int(round(self.length/10,0)), int(round(self.width/10,0)),
                                    self.concr_grade_list[int(self.concr_grade)-1], self.concr_cover, self.scale)
        sec_label = AllplanBasisElements.TextElement(self.common_prop, text_prop,
                                                      text,
                                                      self.ref_point + AllplanGeo.Point2D(\
                                                      self.length/2, self.height + self.slab_thickness + self.max_stt + 750*self.factor))
        return sec_label
    ...
    def On_analysis(self):
        ...
        self.moved_point = AllplanGeo.Point2D(-self.delta, 1000 - self.base_point.Values()[2]) + self.ref_point
        self.label_point = Points.addy_2d(self.ref_point - self.moved_point, 1000)
    ...
    def DimensionLines(self):
        ...
        dim_line = AllplanBasisElements.DimensionLineElement(dim_points, AllplanGeo.Vector2D(self.length + 150*self.factor, 0), AllplanGeo.Vector2D(0, 1), dim_prop)
        dim_line.SetCommonProperties(self.common_prop)
        dimen_lines.append(dim_line)
        ...
        dim_line = AllplanBasisElements.DimensionLineElement(dim_points, AllplanGeo.Vector2D(self.length + 300*self.factor, 0), AllplanGeo.Vector2D(0, 1), dim_prop)
        dim_line.SetCommonProperties(self.common_prop)
    ...
    def ElevationPoints(self):
        ...
        elevation_x1 = AllplanBasisElements.ElevationElement(elevation, AllplanGeo.Vector2D(-300*self.factor, 0), AllplanGeo.Vector2D(0, 1), elevation_prop)
    ...
    def Sec_def_data(self, distance=0):
        ...
        section_def_data.SectionBody = AllplanGeo.Polyhedron3D.CreateCuboid(
                                            ...
                                            AllplanGeo.Point3D(1000, 1000, 1000))
        section_def_data.DirectionVector = AllplanGeo.Vector3D(self.base_point, self.column_points[3])
